...
1. **Codex 리뷰가 최우선** — 진짜 3자(타 모델) 관점. 매 사이클 `which codex`로 CLI 설치 여부를 확인하고, 설치되어 있으면 Codex에게 리뷰를 요청해 그 결과를 우선 반영한다 (Codex는 AGENTS.md에 따라 리뷰 전용). 미설치 시 Claude 셀프 리뷰로 대체하되 사용자에게 설치를 안내한다.
2. **GitHub Issues 확인** — 작업 도중 `gh issue list -R devdongin/devdongin.github.io` 로 이슈란을 확인하고, 열린 이슈를 작업 항목으로 반영한다.
3. Claude 셀프 HR/CTO 리뷰 (아래 절차).

## AI 리뷰 스트립 자동 갱신

랜딩 페이지 최상단 `#ai-review` 스트립은 **AI가 선동인에 대해 자동 판단한 한줄 평가**다. 의미 있는 변경·리뷰 사이클마다 평가문과 갱신 시각(KST)을 업데이트한다. "AI가 자동 생성·갱신한다"는 명시 문구는 유지한다.
