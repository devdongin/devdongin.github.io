...
1. **Codex 리뷰가 최우선** — 진짜 3자(타 모델) 관점. 매 사이클 `which codex`로 CLI 설치 여부를 확인하고, 설치되어 있으면 Codex에게 리뷰를 요청해 그 결과를 우선 반영한다 (Codex는 AGENTS.md에 따라 리뷰 우선이며 큰 변경은 사전 통지 후 직접 구현 가능). 미설치 시 Claude 셀프 리뷰로 대체하되 사용자에게 설치를 안내한다.
