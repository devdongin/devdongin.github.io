...
1. **Codex 리뷰가 최우선이며, 수용적으로 반영한다** — 진짜 3자(타 모델) 관점. 매 사이클 `which codex`로 CLI 설치 여부를 확인하고, 설치되어 있으면 Codex에게 리뷰를 요청해 그 결과를 **기본적으로 채택(수용적 반영)**한다 — 반박·방어보다 수정이 우선. 단, 민감정보 금지 규칙과 충돌하는 제안만 예외로 하고 사유를 기록한다. (Codex는 AGENTS.md에 따라 리뷰 전용.) 미설치 시 Claude 셀프 리뷰로 대체하되 사용자에게 설치를 안내한다.
