...
Vision AI 영상보안 솔루션을 연구실 모델에서 고객사 운영 환경까지 제품화한 기록을 담았습니다.
추론 SDK · Windows 클라이언트 · 커널 드라이버 · 배포 파이프라인, 그리고 운영 장애 대응까지.

## 무엇으로 만들었나

의존성 없는 **정적 HTML 한 장**입니다. 빌드 단계도, 프레임워크도, 번들러도 없습니다.
외부 요청은 폰트 CDN(Pretendard) 하나뿐이고, 나머지는 전부 인라인 CSS·JS와 저장소 안의 이미지입니다.
...
| AI REVIEW | 3시간마다 | Claude Code가 최신 통계를 근거로 평가 문단 재작성 |
